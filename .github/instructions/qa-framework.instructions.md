...
This project uses `@keber/qa-framework` v1.11.3 for spec-driven automated testing.
...
12. **Spanish accents are mandatory, not optional (BLOCKING):** `conventions.language` in `qa/qa-framework.config.json` is `es` — every QA artifact written in Spanish (specs, test plans, memory files, defect reports, `AGENT-NEXT-STEPS.md`, `README.md` rows) must carry correct accents (tildes, `ñ`) as real Spanish prose requires. Stripping accents to plain ASCII is a defect, not a house style, and self-reported compliance is not sufficient evidence — when asked to confirm, produce the actual command output (e.g. `rg -c "[áéíóúÁÉÍÓÚñÑüÜ]" <file>`), not just a claim. A pre-commit hook (`.githooks/pre-commit`, see `.githooks/README.md`) gates this for staged `.md` files; do not bypass it with `--no-verify`.
13. **Automation Constitution (BLOCKING):** Code under `qa/07-automation/e2e/` follows the rules in `.github/skills/qa-automation/references/constitution.md` (imports from `fixtures/pom/test-options.ts` only, `z.strictObject()` schemas, no XPath, no hard waits, single-tag rule, etc.). A `PreToolUse` hook (`.claude/scripts/enforce_constitution.py`) blocks a violating `Write`/`Edit`/`MultiEdit` mechanically before it lands, and `eslint-rules/scaffold-plugin.mts` catches the same violations in `npm run lint`/CI. If the hook blocks a write, fix the violation — do not ask the user to disable the hook.
