 ...
 *
 * Pre-requisites:
 *   - If the app has login: valid storageState in .auth/user-default.json
 *     (run `npx playwright test --project=setup` first). If the app has no
 *     login (check qa/qa-framework.config.json -> testUsers and qa/memory/
 *     first), leave STATE_FILE as-is - the existence check below skips it.
...
async function run() {
  ...
  // No-login apps have no STATE_FILE to load - pass storageState only if it exists.
