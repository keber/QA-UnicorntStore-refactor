...
## Supplementary reference: automation feasibility

`references/priority-and-feasibility.md` has an equivalent priority decision tree plus a standalone
"Automation Feasibility Criteria" table (Fully / Partially / Not automatable) - consult it when
classifying a TC's `Tipo` column (Manual / Automatizado / Ambos / Bloqueado) if the main template's
Automation Feasibility Rules table isn't specific enough for the case at hand.
