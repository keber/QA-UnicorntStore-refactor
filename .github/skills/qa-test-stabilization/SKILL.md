...
runner, e.g. an ad-hoc diagnosis script) must be confirmed via `npx playwright test` before making
any decision about a defect's status. A standalone script and the official suite can produce
opposite, independently-reproducible results for the exact same scenario — so a standalone result
alone is never sufficient to close or reopen a defect. Always reproduce through the official
runner before acting.
...
The confirmation that "the app violates the spec" must be based on the
official suite's result (`npx playwright test`), not on an isolated or
standalone script.
...
| Official suite is source of truth | `npx playwright test` run with the project's real config and fixtures is the only valid source of truth to determine a test's or defect's status. Standalone diagnostic scripts (Node+Playwright run outside the project's test runner) may only provide complementary diagnostic evidence — they can never be the basis for closing a defect or flipping a `test.fail()` to a passing assertion |
