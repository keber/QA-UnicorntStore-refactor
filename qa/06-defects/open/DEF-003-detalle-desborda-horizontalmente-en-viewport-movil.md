...
| Browser | Chromium (via Playwright, Stage 5 second-pass automation) |
...
|-------|-------------------|--------|
| TC-CAT-DETALLE-047 | Automatizado con `test.fixme()` documentando el ancho esperado (`scrollWidth <= viewportWidth + 1`) | Reactivar cuando se corrija DEF-003 |
...
Cuando se corrija:
1. Remover `test.fixme()` de TC-CAT-DETALLE-047 en `qa/07-automation/e2e/tests/catalogo/detalle.spec.ts`.
