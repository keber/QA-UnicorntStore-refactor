 ...
 * redefine them inline (enforced by
 * `.claude/scripts/enforce_constitution.py`'s `no-inline-universal-invalids`
 * rule). Field-specific boundary/range violations (e.g., out-of-range
 * values for a `number` constrained to `1..5`) may stay inline in the spec
 * file.
 *
 * The file is `.ts` (not `.json`) because JSON cannot represent
 * `undefined`. Tuples are `as const` so the array is `readonly` and values
...
export const INVALID_UUID_VALUES = ['not-a-uuid', '', 123, null, undefined] as const;
